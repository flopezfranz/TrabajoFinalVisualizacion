...
Este trabajo es una primera aproximación a la lectura de los datos relevados por la Encuesta, y que esperamos puedan seguir analizándose en el marco del trabajo en el Programa Punto Digital de la Secretaría de Innovación Pública de la Jefatura de Gabinete de Ministros. Los mayores avances que pudimos realizar, se refieren a la presentación de los datos recabados, y algunos cruces entre variables. 

**Presentación de los datos**

Más del 25% de las personas encuestadas, participa de los Puntos Digitales instalados en la provncia de Buenos Aires, seguidos por las provincias de Córdoba y Entre Ríos. 
...
EncPer %>%
  ...
  arrange(desc(Total)) %>% 
  knitr::kable(col.names = c("Provincia", "Total","Porcentaje")) %>% 
...
EncPer %>%
  ggplot(aes(Provincia,fill=Provincia))+
 geom_bar() +
    scale_fill_manual(values=c("yellow", "purple4","brown","green4", "purple4","black","green4", "purple4","black","green4", "purple4","black","green4", "purple4","yellow","green4", "purple4","orange","green4", "purple4","black","green4", "yellow","yellow"))+
   labs(title="total",
       y="rtas",
       x="Pcia") +
  scale_y_continuous(breaks = seq(0,2000,10))+
   theme_dark(base_size = 16)+
  theme(legend.position = "bottom",legend.background = element_rect(fill="pink",color="blue",linewidth = 1))
...
RANGO ETARIO
Si bien el promedio de edad es, como mecionamos, de 35 años, la mayoría de las personas encuestadas tiene entre 16 y 26 años. 
...
mutate(porcentaje = (frecuencia/ sum(frecuencia)) * 100) %>% 
 knitr::kable(col.names = c("Nivel Educativo","Total","Porcentaje")) %>% 
 kable_material_dark("hover",full_width = F) %>% 
  print()
...
arrange(desc(frecuencia)) %>% 
mutate(porcentaje = (frecuencia/ sum(frecuencia)) * 100) %>% 
   knitr::kable(col.names = c("Trabajo","Total","Porcentaje")) %>% 
 kable_material_dark("hover",full_width = F) %>% 
  print()
...
Satis<-EncPer %>% 
 ...
 mutate(Porcentaje = (Total/ sum(Total)) * 100) %>% 
  arrange(desc(Porcentaje)) %>% 
  knitr::kable(col.names = c("Grado de satisfacción con la democracia","Total","Porcentaje")) %>% 
